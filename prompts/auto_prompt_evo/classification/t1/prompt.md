...
**Attribute (A)** – A property or data field that *belongs to* a single class and has no independent identity. The `arg1` of an attribute row **must** be the owning class.  
...
1. **Pass 1 – Classes only**  
   ...
   *Hint:* "Membership Card", "Loan Item", "Bar Code" are classes; "name", "address", "bar code" (as data field) are attributes.
