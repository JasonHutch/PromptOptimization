...
- **No verbs or actions as classes** – never create a Class (C) for a verb, action, or process phrase (e.g., "borrow", "issue", "rent", "reserve", "scan", "read", "enter", "stamp", "search", "identify", "update of records", "issues", "shows", "kept", "denoted", "extend", "scanned", "entered", "read", "stamped", "searched", "identified", "made up of", "has a title language", "has a title and author", "customer is known as a member", "language tapes and books are two types of loan items"). These are candidates for AS/AC or should be ignored.
- **No attribute-like nouns as classes** – never create a Class (C) for noun phrases that are attribute-like (see the exclusion list below). These must be modeled as Attributes (A) of their owning Class.
- **No aggregation of attributes/values** – never create AG rows where `arg1` is an attribute or value. AG is strictly between two Classes (part and whole).
- **No inheritance between non-classes** – never create I rows where `arg1` or `arg2` is not a Class from Pass 1.
- **Pass 1 Class Filter** – In Pass 1, ONLY output Class (C) rows. Do not output A, V, AS, AC, AG, I rows during Pass 1. The final output is the union of all passes.

## Exclusion Lists (apply strictly during Pass 1)

**Never create Classes for these attribute-like nouns (model as Attributes instead):**
manufacturer, make, model, price class, price, rental price, rental charge, depreciation, theme, slogan, duration, contact information, website, service charges, payments received, account balances, fee, size, evaluation, status, keynote address, type, grace period, means, void, opened, balance, charges, payment, slogan, theme, duration, location, product, proposal, reviewer, review committee, seminar, booth, invited speaker, participant, exhibitor, observer, staff, service charges, payments received, account balances, contact information, website, fee, size, price, evaluation, type, grace period, means, void, opened, balance, charges, payment, id, bar code, title, author, number of doors, transmission, body style, additional charge, number of subject sections, number of items, loan period, membership status, maximum loan items, loan items constraint, subject, language, level

**Never create Classes for these role/property-holder nouns (model as Attributes or ignore):**
organizer (when only holding properties), domain (when only holding properties), account (when only holding properties for another entity), service provider, venue, conference room, keynote address, trade show material, reviewer, review committee, staff, service charges, payments received, account balances

**Never create Classes for verb/action/process phrases:**
borrow, issue, rent, reserve, manufacture, select, process, sign, pay, send, attend, register, invite, evaluate, review, exhibit, setup-with, submit, take from, return to, bill for, check out, open, cover, check, archive, promote, organize, run, contact, create, record, regard, belong to, add, remove, prepare, give, comprise, request, setup, receive, visit, taken from, returned to, sent to, processed by, reviewed-by, has, scan, read, enter, stamp, search, identify, update of records, issues, shows, kept, denoted, extend, scanned, entered, read, stamped, searched, identified, made up of, has a title language, has a title and author, customer is known as a member, language tapes and books are two types of loan items
...
*Exclusion rule:* Do **not** create classes for noun phrases that appear **only** as property holders or roles without independent relationships or attributes of their own (see Exclusion Lists above).  
*Attribute-like nouns (never classes):* See Exclusion Lists above.  
...
## Critical Reminders
- If a noun phrase is in the Exclusion Lists, it is NOT a Class. Model it as an Attribute of its owner.
- If a phrase is a verb/action, it is NOT a Class. Model it as an Association (AS) or Association Class (AC) if it carries data.
- Aggregation (AG) and Inheritance (I) only link Classes from Pass 1.
- Values (V) only link to Attributes (A) from Pass 2.
- The final CSV is the concatenation of all passes.