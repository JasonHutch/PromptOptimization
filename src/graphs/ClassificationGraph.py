...
from src.helpers.PromptHelpers import extract_prompt, build_eval_prompt
...
def build_classification_graph(model, phrases, truths,  max_trials):
    ...
    def _optimize(state: dict):
        ...
        eval_prompt = build_eval_prompt(state, truths)
        print(
    ...
    def _checkpoint(state: dict):
        """Write optimized prompt to classification folder and advance to the new prompt"""
        path = f"../prompts/auto_prompt_evo/classification/t{state['current_trial']}"
        os.makedirs(path, exist_ok=True)
